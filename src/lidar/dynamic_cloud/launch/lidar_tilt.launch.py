...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('accumulate_time', default_value='1', description='动态点云累积窗口大小'),
        DeclareLaunchArgument('publish_accumulated_dynamic_cloud', default_value='false', description='是否发布累积后的动态点云到 cluster'),
        DeclareLaunchArgument('cluster_tolerance', default_value='0.25', description='欧式聚类距离阈值'),
        DeclareLaunchArgument('min_cluster_size', default_value='6', description='最小聚类点数'),
        ...
        DeclareLaunchArgument('self_color', default_value='R', description='当 /match_info 缺失时使用的己方颜色（R 或 B）'),
        ...
        DeclareLaunchArgument('camera_time_match_threshold', default_value='0.25', description='融合时允许的最大相机/LiDAR 时间差（秒）'),
        DeclareLaunchArgument('detect_queue_max_age', default_value='1.0', description='相机 detect 队列保留的最大秒数'),
        DeclareLaunchArgument('detect_queue_max_size', default_value='20', description='相机 detect 队列最多保留的消息数'),
        DeclareLaunchArgument('use_smoothed_camera_match_point', default_value='false', description='是否允许用平滑后的 KF 输出参与相机匹配'),
